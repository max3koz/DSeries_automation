...
class TestSessionManagement:
    def test_create_session_positive(self, setup_module_max, schedule_window=100, as_run_log_window =100):
    ...
    def test_ping_session_positive(self, setup_module_min, schedule_window=16, as_run_log_window=16):
        session = Session(schedule_window, as_run_log_window)
    ...
    def test_delete_session_positive_1(self, setup_class, schedule_window=16, as_run_log_window=16):
        session = Session(schedule_window, as_run_log_window)
        session_response_code = Session.delete_session(session.session_id)
        logging.info(session_response_code)
        assert_that(session_response_code, "Error: the session ID is not exists").is_equal_to(200)
    ...
    def test_delete_session_positive_2(self, setup_class, schedule_window=16, as_run_log_window=16):
        session = Session(schedule_window, as_run_log_window)
        session_response_code = Session.delete_session(session.session_id)
        logging.info(session_response_code)
        assert_that(session_response_code, "Error: the session ID is not exists").is_equal_to(200)
